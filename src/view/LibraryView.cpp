...
void LibraryView::toggleMode()
{
    setMode(_mode == Mode::Directory ? Mode::ArtistAlbum : Mode::Directory);
}
...
void LibraryView::rebuild()
{
    ...
    if (_mode == Mode::Directory) buildDirectoryTree();
    else                          buildArtistAlbumTree();
...
void LibraryView::buildDirectoryTree()
{
        ...
        [&](std::size_t parentIdx, std::string const & label, int depth) -> std::size_t
    {
        ...
        n.label    = label;
        n.depth    = depth;
        n.expanded = (depth == 0);   // top-level open by default
        n.parent   = parentIdx;
    ...
    for (auto const & t : _library->tracks())
    {
        ...
        if (!root.empty())
        {
            ...
            if (rel.empty() || rel.native()[0] == '.') rel = t.path;
        }
        else
        {
            rel = t.path;
        ...
        for (auto const & p : rel) parts.push_back(toNfc(p.string()));
        ...
        if (parent == kNoIdx) _roots.push_back(idx);
        else                  _nodes[parent].children.push_back(idx);
    }
}
...
void LibraryView::sortNodeChildren()
{
    auto cmp = [&](std::size_t a, std::size_t b)
    {
        ...
        if (na.kind != nb.kind) return na.kind == Node::Kind::Group;
...
void LibraryView::collectTracks(std::size_t nodeIdx, std::vector<TrackInfo> & out) const
{
    ...
    for (auto child : n.children) collectTracks(child, out);
}

bool LibraryView::expandPath(std::size_t nodeIdx)
{
    if (nodeIdx >= _nodes.size()) return false;
    bool changed = false;
...
void LibraryView::locate(std::filesystem::path const & path)
{
    ...
    for (std::size_t i = 0; i < _nodes.size(); ++i)
    {
        ...
        if (n.kind != Node::Kind::Track || !n.track) continue;
        if (n.track->path.string() != target) continue;

        bool changed = expandPath(i);
        if (changed) recomputeVisible();
...
void LibraryView::draw(ventty::Window & window)
{
    ...
    // Right border (panel separator handled by Application)
    ...
    std::string header = " Library [";
    header += (_mode == Mode::Directory) ? "Dir" : "Artist";
    header += "]";
    if (_library)
    {
        header += " (" + std::to_string(_library->size()) + ")";
    }
...
{
    if (_visible.empty()) return false;

    auto const visSize = static_cast<int>(_visible.size());

    if (event.key == Key::Up)
    {
        if (_selectedIndex > 0) { _selectedIndex--; scrollToSelected(); }
        return true;
    }
    if (event.key == Key::Down)
    {
        if (_selectedIndex < visSize - 1) { _selectedIndex++; scrollToSelected(); }
        return true;
    }
    ...
    {
        if (current.kind == Node::Kind::Group)
        {
            // Toggle expansion on Enter for groups; leave queue actions to
            // explicit selections of track nodes.
        ...
        if (_onSend)
        {
            ...
            _onSend(std::move(out), /*replace=*/!event.shift);
        }
        return true;
    }
